<template>
    <v-app id="mur" class="mur">
        ...
        <div class="ml-12">
        <div class="flexible">
            <h1 class="ma-4 cut-text">Fil d'actualité</h1>
        </div>
        <router-view></router-view>
            <v-container class="maclass">
                <v-btn class="ma-3 createPost" @click="afficheForm">Créer un post</v-btn>
                ...
                <v-card-image class="mur__post ma-3 mt-6" v-for="(post, index) in allPosts" v-bind:key="index">
                  ...
                  <div class="post">
                    ...
                    <v-card-text class="py-0">
                        <v-btn fab title="like ?" class="ma-3 hover" color="rgb(215,215,215)"  @click="likePost(post.id, post.likes)">
                                <v-icon >mdi-heart-outline</v-icon>
                        </v-btn> 
                        {{ post.likes }}
                        <v-icon>mdi-heart-outline</v-icon>
                        <!--Action pour visualiser les commentaires-->    
                        <v-btn text @click="afficheCom(post.id)" class="hover" title="voir les commentaires">
                    ...
                    <div class="mur__comments" v-if="postId === post.id">
                        <v-card class="mur__comments--ind my-1 mx-2 pa-3 " color="rgba(255,215,215,0.3)" v-for="(comment, index) in allComments" v-bind:key="index" outlined>
                          ...
                          <v-btn v-if="!afficheFrmCm" title="commenter le post" class="ma-2 hover" @click="afficheFormCom()">Commenter</v-btn>
                                                  <!--new comment - form--> 
